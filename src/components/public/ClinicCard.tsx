  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        {clinic.verified && (
          <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-50 text-emerald-600 text-[12px] font-semibold uppercase tracking-wide rounded-full border border-emerald-100">
            Verified
          </span>
        )}
      </div>
      {clinic.description && (
        <p className="text-sm text-gray-500 mt-3 leading-relaxed line-clamp-2">{clinic.description}</p>
      )}
    </Link>
  )
}
