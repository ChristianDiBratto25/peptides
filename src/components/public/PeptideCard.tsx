...
export default function PeptideCard({ peptide }: { peptide: Peptide }) {
  return (
    ...
    >
      ...
      {peptide.category && (
        <span className="inline-block mt-2 px-2.5 py-0.5 bg-[#f3ecfe] text-[#7f21f6] text-[12px] font-medium uppercase tracking-wide rounded-full">
          {peptide.category}
        </span>
      )}
      {peptide.summary && (
        <p className="text-sm text-gray-500 mt-3 leading-relaxed line-clamp-2">{peptide.summary}</p>
      )}
    </Link>
  )
}
